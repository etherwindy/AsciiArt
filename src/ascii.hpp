...
private:
    ...
    // 输出带颜色的 ASCII 图片
    void print_ascii_color(const unsigned char *data, const int &width,
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                ...
                std::cout << "\e[38;2;" << r << ";" << g << ";" << b << "m"
                          << ascii_chars[gray_bits][ascii_index];
            }
    ...
    // 输出灰度 ASCII 图片
    void print_ascii_gray(const unsigned char *data, const int &width,
                          const int &height, const int &channels) {
        const int gray_levels = pow(2, gray_bits);
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
    ...
    void print_ascii_gray_from_gray(const unsigned char *data, const int &width,
                                    const int &height, const int &channels) {
        const int gray_levels = pow(2, gray_bits);
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
...
public:
    AsciiGenerator(const int &gray_bits, const float &gamma = 1.0,
                   const bool &enable_color = false,
                   const bool &enable_alpha = false,
                   const bool &gray_reverse = false)
        : enable_color(enable_color), gray_bits(gray_bits),
          enable_alpha(enable_alpha), gray_reverse(gray_reverse) {
        init_gamma_table(gamma);
    }

    // 输出 ASCII 图片
    void print_ascii(const unsigned char *data, const int &width,
                     const int &height, const int &channels) {
