...
private:
    ...
    inline int get_block_code(const unsigned char *data, const int &width,
    ...
    inline int get_block_code_from_gray(const unsigned char *data,
    ...
    inline void get_block_rgb(const unsigned char *data, const int &width,
                              const int &height, const int &channels,
    ...
    void print_unicode_block_gray(const unsigned char *data, const int &width,
                                  const int &height, const int &channels) {
        ...
        for (int i = 0; i < block_height; i++) {
            for (int j = 0; j < block_width; j++) {
                int block_code =
                    get_block_code(data, width, height, channels, i, j);
    ...
    // 输出带颜色的 Unicode 盲文字符
    void print_unicode_block_color(const unsigned char *data, const int &width,
                                   const int &height, const int &channels) {
        const int block_width  = width / 2;
        const int block_height = height / 2;
    ...
    // 输出不带颜色的 Unicode 盲文字符
    void print_unicode_block_gray_from_gray(const unsigned char *data,
                                            const int &width, const int &height,
                                            const int &channels) {
        const int block_width  = width / 2;
        const int block_height = height / 2;
        ...
        for (int i = 0; i < block_height; i++) {
            for (int j = 0; j < block_width; j++) {
                int block_code = get_block_code_from_gray(data, width, height,
                                                          channels, i, j);
                wchar_t block_char = get_block_char(block_code);
...
public:
    UnicodeBlockGenerator(const int &threshold_gray, const int &threshold_alpha,
    ...
    // 输出 Unicode 盲文字符
    void print_unicode_block(const unsigned char *data, const int &width,
                             const int &height, const int &channels) {
